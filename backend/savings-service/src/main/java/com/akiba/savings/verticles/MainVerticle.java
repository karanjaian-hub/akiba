...
public class MainVerticle extends VerticleBase {
  ...
  private Future<Void> startHttpServer(
    SavingsRepository repository, RedisAPI redis, RabbitMQClient rabbitMQ) {
    ...
    int port = Integer.parseInt(System.getenv().getOrDefault("SERVICE_PORT", "8087"));
  ...
  private Pool buildPool() {
    boolean ssl = !System.getenv().getOrDefault("REDIS_TLS", "false").equals("false");
    ...
    if (ssl) {
      connectOptions.setSslOptions(
        new io.vertx.core.net.ClientSSLOptions().setTrustAll(true));
    }
  ...
  private RedisAPI buildRedis() {
    ...
    String tls      = System.getenv().getOrDefault("REDIS_TLS", "false");
