...
public class HttpVerticle extends VerticleBase {
  ...
  public Future<?> start() {
    ...
    return vertx.createHttpServer()
      ...
      .listen(port)
      .onSuccess(v -> System.out.println("[NotificationService] HTTP listening on port " + port))
      .mapEmpty();
  }
  ...
  private void handleUpdatePreferences(RoutingContext ctx) {
    ...
    prefsRepo.upsertPreferences(userId, ctx.body().asJsonObject())
      .onSuccess(v -> ctx.response().setStatusCode(204).end())
      .onFailure(err -> sendError(ctx, 500, err.getMessage()));
  }
  ...
  private UUID extractUserId(RoutingContext ctx) {
    String header = ctx.request().getHeader("X-User-Id");
    if (header == null) {
