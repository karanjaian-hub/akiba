...
public class HealthCheckHandler {
    ...
    public HealthCheckHandler(Vertx vertx, Pool pgPool, RabbitMQClient rabbit, RedisAPI redis, String serviceName) {
        this.vertx       = vertx;
        this.pgPool      = pgPool;
        this.rabbit      = rabbit;
        this.redis       = redis;
    ...
    private Future<String> raceWithTimeout(Future<String> check, long timeoutMs, String label) {
        ...
        long timerId = vertx.setTimer(timeoutMs, id -> timeoutPromise.tryComplete("DOWN"));
        check.onComplete(result -> {
            vertx.cancelTimer(timerId);
            timeoutPromise.tryComplete(result.failed() ? "DOWN" : result.result());
        });
        return timeoutPromise.future();
    }

    private JsonObject buildResponse(long uptimeSeconds, String dbStatus, String rabbitStatus, String redisStatus) {
